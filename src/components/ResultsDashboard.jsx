            ...
            <div className="auditor-split-view" style={{
                ...
                <div className="left-pane" style={{
                    ...
                    <div className="document-content" style={{ whiteSpace: 'pre-wrap', lineHeight: '1.6' }}>
                        ...
                        */}
                        {text.split(new RegExp(`(${results.keyPhrases.filter(p => p.found).map(p => p.text.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&')).join('|')})`, 'gi')).map((part, i) => {
                            const isMatch = results.keyPhrases.some(p => p.found && p.text.toLowerCase() === part.toLowerCase());
                            return isMatch ?
